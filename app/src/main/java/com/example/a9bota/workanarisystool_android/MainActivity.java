...
public class MainActivity extends Activity
     ...
     * 前提条件は次のとおりです。
     * GooglePlay Servicesがインストールされ、
     * アカウントが選択され、
     * デバイスに現在オンラインアクセスがあります。
     * 前提条件のいずれかが満たされていない場合、
     * アプリは必要に応じてユーザーにプロンプトを表示します。
     */
     ...
     * そうでない場合、アカウントピッカーダイアログがユーザーに表示されます。
     * アカウントを資格情報オブジェクトで使用するように設定するには、
     * アプリにGET_ACCOUNTS権限が必要です。これは、まだ存在しない場合はここで要求されます。
     * AfterPermissionGrantedアノテーションは、
     * GET_ACCOUNTSパーミッションが付与されるたびにこの関数が自動的に再実行されることを示します。
     */
     ...
     */
    /**
     * Google Sheets API呼び出しを処理する非同期タスク。
     * API呼び出しを独自のタスクに配置すると、UIの応答性が維持されます。
     */
